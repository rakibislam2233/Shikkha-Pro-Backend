...
const generateQuiz = async (
  ...
  try {
    ...
    const quizData: ICreateQuizRequest = {
      ...
      description: `A ${request.difficulty} level quiz on ${
        request.topic
      } for ${
    ...
    throw new ApiError(
      ...
      `Failed to generate quiz: ${error.message || error}`
    );
  }
};
...
const submitAnswer = async (request: ISubmitAnswerRequest, userId: string) => {
  ...
  if (attempt.timeLimit) {
    ...
    if (timeElapsed > attempt.timeLimit * 60) {
      // timeLimit is still in minutes
...
const submitQuizAnswer = async (
  ...
  if (existingAttempt) {
    ...
    const timeDiff =
      existingAttempt.completedAt.getTime() -
  ...
  const startedAt =
    request.startedAt || new Date(Date.now() - (request.timeSpent || 0) * 1000); // timeSpent is in seconds
